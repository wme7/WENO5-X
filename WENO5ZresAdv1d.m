function res = WENO5ZresAdv1d(w,flux,dflux,S,dx)
% *************************************************************************
...
% Constants
d0n = 1/10; d1n = 6/10; d2n = 3/10; epsilon = 1e-40; tau5 = abs(B0n-B2n);

% Alpha weights 
alpha0n = d0n.*(1+tau5./(B0n + epsilon));
alpha1n = d1n.*(1+tau5./(B1n + epsilon));
alpha2n = d2n.*(1+tau5./(B2n + epsilon));
...
% Constants
d0p = 3/10; d1p = 6/10; d2p = 1/10; epsilon = 1e-40; tau5 = abs(B0p-B2p);

% Alpha weights 
alpha0p = d0p.*(1+tau5./(B0p + epsilon));
alpha1p = d1p.*(1+tau5./(B1p + epsilon));
alpha2p = d2p.*(1+tau5./(B2p + epsilon));
